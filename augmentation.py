...
def LineCut(img, **kwarg):
    img_arr = np.array(img)
    for i in range(0, img_arr.shape[0]):
        for j in range(0, img_arr.shape[1]):
            r = random.random()
            if sum(img_arr[i][j])<300 and r<0.3:
                img_arr[i][j] = np.ones(len(img_arr[i][j]))*(255-int(r*100))
...
def Posterize(img, v, max_v, bias=0):
    v = int(8 - _round_parameter(v, max_v) + bias)
...
def soft_augment_pool():
    # Test
    augs = [#(LineCut, None, None),
            #(AutoContrast, None, None),#去掉了原来直方图中最暗和最亮的各20%，剩下的像素值然后再映射到[0，255]的颜色空间上。
            (Rotate, 30, None),
            #(Brightness, 1.8, 0.1),#变量变暗
            #(Color, 1.8, 0.1),#饱和度调节
            #(Contrast, 1.8, 0.1),#变模糊变清晰
            #(CutoutConst, 40, None),
            #(Posterize, 4, 0),#将每个颜色通道上变量bits对应的低(8-bits)个bit置0
            #(Sharpness, 1.8, 0.1),#锐度：反映图像平面清晰度和图像边缘锐利程度的一个指标。值较小时，图像偏模糊,值较大时边缘更加锐利。
            ]
    return augs
...
class EasyAugment(object):
    def __init__(self, n, m, resample_mode=PIL.Image.BICUBIC):
        global RESAMPLE_MODE
        RESAMPLE_MODE = resample_mode
        self.n = n
        self.m = m
        self.augment_pool = easy_augment_pool()
...
class RandAugment(object):
    ...
    def __call__(self, img, weights=None):
        ops = random.choices(self.augment_pool, k=self.n, weights=weights)
        for op, max_v, bias in ops:
            prob = np.random.uniform(0.2, 0.8)
